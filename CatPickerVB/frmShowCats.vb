...
Public Class frmShowCats
  ...
  Dim catDetailbs As BindingSource
...
endd:
  ...
  Private Sub clearControlBinding()
    ...
    txtCatDetailsCatId.DataBindings().Clear()
...
abort:
  ...
  Private Sub txtCatDetailsCatId_TextChanged(sender As Object, e As EventArgs) Handles txtCatDetailsCatId.TextChanged
    ...
    detailVM = catDetailBll.getAll(parmList)
    catDetailbs.DataSource = detailVM.catNoteList


    If (Not IsNothing(detailVM.catDetailList)) Then
      If (Not IsNothing(detailVM.catNoteList)) Then
        txtCatNotes.DataBindings().Clear()
        txtCatNotes.DataBindings.Add(New System.Windows.Forms.Binding("Text", catDetailbs.DataSource, "catNote", True, 2))
      End If
    End If
  ...
  Private Sub AddCatNotes_Click(sender As Object, e As EventArgs) Handles AddCatNotes.Click
    ...
    '********** Purpose: Add random simulated cat notes to catPicker/catNotes table. The text of the cat note is a random joke from an API
    ...
    For Each id In catIdList
      ...
      'numOfRecs = 1 ' delete/disable this to run cat note adder, use this to test cat note adder without multiple api requests.
      paramList.Clear()
      ...
      For rec As Integer = 1 To numOfRecs
        ...
        'creat a note
        catNoteBLL.createCatNotes(New Object)

        'get a note
        'catNote = catNoteBLL.createCatNotes
        ...
        '  paramList.Add(ValueTuple.Create("note", catNote.noteSetup))
